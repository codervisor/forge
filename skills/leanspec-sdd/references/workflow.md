...
3. **Populate all known fields at creation time** — title, content, priority, tags, parent, depends-on.
4. Write clear requirements and acceptance criteria.
5. Validate token count
   - MCP: `tokens`
   - CLI: `lean-spec tokens <spec>`
6. If >2000 tokens, split into separate files (e.g. DESIGN.md, TESTING.md) or extract to references.
...
3. **Verify actual implementation** — check git commits, file changes, tests.
4. Update status to `complete` only after validation passes.

## Quick Checklist

- Discovery done first
- Spec created or verified
- All known fields populated at creation time
...
- Actual implementation verified before marking complete
